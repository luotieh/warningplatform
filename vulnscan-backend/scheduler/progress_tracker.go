...
func (pt *ProgressTracker) AppendFindings(findings []*engine.Finding) {
	pt.mu.Lock()
	pt.findings = append(pt.findings, findings...)
	pt.mu.Unlock()
}

func (pt *ProgressTracker) GetFindings() []*engine.Finding {
	pt.mu.RLock()
	defer pt.mu.RUnlock()
	out := make([]*engine.Finding, len(pt.findings))
