...
type Runner struct {
	...
	enricher    *engine.TargetEnricher
...
func NewRunner(db *gorm.DB, task model.ScanTask, eventBus *EventBus, opts ...RunnerOption) *Runner {
	r := &Runner{
		...
		adaptive:      engine.NewAdaptiveController(2, 50),
		circuit:       engine.NewCircuitBreaker(5, 30*time.Second),
		dedup:         engine.NewFindingDeduplicator(false),
		prioritizer:   engine.NewModulePrioritizer(),
		stream:        engine.NewFindingStream(1024),
		enricher:      engine.NewTargetEnricher(),
		checkpoint:    NewCheckpointManager(db),
		strategy:      engine.NewStrategyEngine(),
	}
...
func (r *Runner) persistAndPublish(findings []*engine.Finding, stage, moduleID string) {
	if len(findings) == 0 {
		return
	}
...
func buildTargets(task model.ScanTask) []*engine.Target {
...
func appendUniqueTargets(existing, newTargets []*engine.Target) []*engine.Target {
	seen := make(map[string]struct{})
	for _, t := range existing {
		seen[targetKey(t)] = struct{}{}
	}
	result := existing
	for _, t := range newTargets {
		...
		if _, ok := seen[key]; ok {
			for i, e := range result {
				if targetKey(e) == key && t.URL != "" && e.URL == "" {
					result[i] = t
					break
				}
			}
...
func targetKey(t *engine.Target) string {
...
func extractDetectedProducts(findings []*engine.Finding, config map[string]interface{}) []string {
	seen := make(map[string]struct{})
...
func extractDetectedWAFs(findings []*engine.Finding) []string {
	seen := make(map[string]struct{})
	for _, f := range findings {
