...
func isReconFinding(f *engine.Finding) bool {
...
func findingToRecord(task model.ScanTask, f *engine.Finding) model.ScanFinding {
	target := ""
	port := 0
	protocol := ""
	if f.Target != nil {
...
func computeDedupKey(taskID string, f *engine.Finding) string {
	target := ""
	port := 0
	protocol := ""
	if f.Target != nil {
