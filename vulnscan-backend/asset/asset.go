	...
	return authorize.RegisterRoutes(e.Group("/asset"), []authorize.Route{
		{
			...
			Children: []authorize.Route{
				...
				{Name: "资产行业范围", Path: "industry-scope", Method: "GET", Handler: m.enrichHandler.IndustryScope, Enabled: true},
				{Name: "资产单位类型范围", Path: "unit-type-scope", Method: "GET", Handler: m.enrichHandler.UnitTypeScope, Enabled: true},
				{Name: "同步在线状态", Path: "sync-online-status", Method: "POST", Handler: m.enrichHandler.SyncOnlineStatus, Enabled: true},
				{Name: "地址可达性检测", Path: "probe-address", Method: "POST", Handler: m.enrichHandler.ProbeAddress, Enabled: true},
				...
				{Name: "资产首页截图", Path: ":id/screenshot", Method: "GET", Handler: m.enrichHandler.AssetScreenshot, Enabled: true},
