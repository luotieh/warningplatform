...
func (s *ServiceSchedule) RunNow(id string) (string, error) {
	var item model.ScanSchedule
	if err := s.session().Where("id = ?", id).First(&item).Error; err != nil {
		return "", err
	}
	...
	if err := s.session().Create(&task).Error; err != nil {
		return "", err
	}

	s.session().Model(&model.ScanSchedule{}).Where("id = ?", id).Updates(map[string]any{
		"last_task_id": taskID,
