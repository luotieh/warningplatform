...
func (p *SmartTargetPropagator) Propagate(existing, newTargets []*core.Target, findings []*core.Finding) []*core.Target {
	if len(newTargets) == 0 {
		return existing
	}
...
func (d *TargetDeduplicator) SmartDedup(targets []*core.Target) []*core.Target {
...
func (d *TargetDeduplicator) mergeWildcards(targets []*core.Target) []*core.Target {
	domainMap := make(map[string]*core.Target)
	...
	result := make([]*core.Target, 0, len(domainMap))
...
func (d *TargetDeduplicator) aggregateIPRanges(targets []*core.Target) []*core.Target {
	ipGroups := make(map[string][]*core.Target)
	var nonIPs []*core.Target
	...
	result := make([]*core.Target, 0, len(nonIPs)+len(targets))
...
func (d *TargetDeduplicator) dedupByService(targets []*core.Target) []*core.Target {
...
func (d *TargetDeduplicator) mergeTargetInfo(dest, src *core.Target) {
	if dest.IP == "" && src.IP != "" {
		dest.IP = src.IP
	}
	if dest.URL == "" && src.URL != "" {
...
func (p *TargetPrioritizer) SortByRisk(targets []*core.Target, findings []*core.Finding) []*core.Target {
...
func (p *TargetPrioritizer) baseScore(t *core.Target) float64 {
	score := 10.0
...
func (p *TargetPruner) Prune(targets []*core.Target, findings []*core.Finding) []*core.Target {
