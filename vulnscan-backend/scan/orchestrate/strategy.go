...
func (se *StrategyEngine) DetectTechStack(findings []*core.Finding, targets []*core.Target) TechStack {
...
func (se *StrategyEngine) FilterModules(modules []core.ScanModule, stack TechStack) []core.ScanModule {
	...
	highPri := make([]core.ScanModule, 0, len(modules))
	normal := make([]core.ScanModule, 0, len(modules))
	...
	rest := make([]core.ScanModule, 0, len(highPri)+len(normal))
...
func containsAnyTech(s string, techs ...string) bool {
	for _, t := range techs {
