...
func (s *stubModule) Run(ctx context.Context, targets []*Target, config map[string]interface{}) (*ModuleResult, error) {
	return &ModuleResult{}, nil
}

func TestPrioritizer_EmptyNoChange(t *testing.T) {
	mp := NewModulePrioritizer()
	mods := []ScanModule{&stubModule{"a"}, &stubModule{"b"}}
...
func TestPrioritizer_HighFindingsFirst(t *testing.T) {
	...
	mods := []ScanModule{&stubModule{"slow_mod"}, &stubModule{"fast_mod"}}
	sorted := mp.SortModules(mods)
...
func TestPrioritizer_FailurePenalty(t *testing.T) {
	...
	mods := []ScanModule{&stubModule{"flaky_mod"}, &stubModule{"reliable_mod"}}
	sorted := mp.SortModules(mods)
...
func TestPrioritizer_MultipleRecords(t *testing.T) {
	...
	mods := []ScanModule{&stubModule{"mod2"}, &stubModule{"mod1"}}
	sorted := mp.SortModules(mods)
	if sorted[0].ID() != "mod1" {
