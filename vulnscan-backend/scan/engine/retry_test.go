...
func (m *mockModule) Run(ctx context.Context, targets []*Target, config map[string]interface{}) (*ModuleResult, error) {
	m.runCount++
...
func TestRunWithRetry_SuccessFirst(t *testing.T) {
	mod := &mockModule{id: "ok_mod", failN: 0, findings: []*Finding{{Title: "found"}}}
...
func TestRunWithRetry_SuccessAfterRetry(t *testing.T) {
	mod := &mockModule{id: "retry_mod", failN: 2, findings: []*Finding{{Title: "found"}}}
	rc := RetryConfig{MaxRetries: 2, InitialBackoff: 10 * time.Millisecond, MaxBackoff: 50 * time.Millisecond}
