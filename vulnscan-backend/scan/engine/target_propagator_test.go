...
func TestTargetPrioritizer_SortByRisk(t *testing.T) {
	...
	findings := []*Finding{
		{
			Target:   &Target{Host: "high.example.com", Port: 80},
			Type:     "vulnerability",
			Severity: "critical",
			Title:    "RCE",
		},
...
func TestTargetPrioritizer_BaseScore(t *testing.T) {
	...
	targetWithURL := &Target{Host: "example.com", Port: 80, URL: "http://example.com"}
	targetWithoutURL := &Target{Host: "example.com", Port: 80}
