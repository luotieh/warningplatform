...
defineOptions({ name: 'IncidentKnowledge' });
...
const columns = computed(() => [
  { title: '标题', key: 'title', minWidth: 200, render: (row: KnowledgeArticle) => h('a', { style: 'color:#2080f0;cursor:pointer', onClick: () => router.push(`/incident/knowledge/${row.id}`) }, row.title) },
  ...
  { title: '操作', key: 'actions', width: 200, fixed: 'right' as const, render: (row: KnowledgeArticle) => h(NSpace, { size: 4 }, () => [
    h(NButton, { size: 'tiny', type: 'info', text: true, onClick: () => router.push(`/incident/knowledge/${row.id}`) }, () => '查看'),
