...
const columns = computed(() => [
  ...
  { title: '操作', key: 'actions', width: 200, fixed: 'right' as const, render: (row: CircularItem) => h(NSpace, { size: 4 }, () => {
    ...
    if (row.status === 'to_be_submit') {
      items.push(h(NButton, { size: 'tiny', type: 'primary', text: true, onClick: () => router.push(`/circular/input/add?id=${row.id}`) }, () => '编辑'));
    ...
    <NCard title="通报录入" size="small">
      <template #header-extra>
        <NSpace :size="8">
          ...
          <NButton size="small" type="primary" @click="router.push('/circular/input/add')">新建通报</NButton>
        </NSpace>
      </template>
