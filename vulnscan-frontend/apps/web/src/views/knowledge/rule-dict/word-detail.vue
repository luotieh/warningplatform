    ...
    <div class="flex gap-3" style="min-height: 500px">
      ...
      <NCard size="small" style="flex: 1; min-width: 0">
        ...
        <NDataTable
          ...
          @update:page="(p: number) => { entryPagination.page = p; fetchEntries(); }"
          @update:page-size="(s: number) => { entryPagination.pageSize = s; entryPagination.page = 1; fetchEntries(); }"
        />
      </NCard>
    </div>
    ...
    >
      <NForm label-placement="left" :label-width="60">
        <NFormItem label="名称" required>
          <NInput v-model:value="catForm.name" placeholder="如：涉黄、涉政" />
        </NFormItem>
        <NFormItem label="描述">
          <NInput v-model:value="catForm.description" type="textarea" :rows="2" />
        </NFormItem>
      </NForm>
      <template #footer>
        <NSpace justify="end">
          ...
          >
            <NUploadDragger>
              <div class="py-4 text-center">
                <div class="text-base font-semibold">点击或拖拽文件到此处</div>
